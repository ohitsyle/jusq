...
/**
 * POST /admin/configurations/manual-export-marketing
 * Manually trigger marketing export (Logs) as a ZIP
 */
router.post('/manual-export-marketing', async (req, res) => {
  try {
    const { exportTypes, dateRange, customStartDate, customEndDate } = req.body;
    ...
    const zipFileName = `marketing_export_${new Date().toISOString().split('T')[0]}.zip`;
    const zipBuffer = zip.toBuffer();
    const base64Data = zipBuffer.toString('base64');
    ...
    logAdminAction({
      adminId: req.adminId || 'system',
      adminName: req.adminName || 'Admin',
      adminRole: req.adminRole || 'marketing',
      department: req.department || 'marketing',
      action: 'Marketing Export',
      description: `exported marketing data (${totalRecords} records): ${exportTypes.join(', ')}`,
      targetEntity: 'config',
      targetId: 'manual-export-marketing',
      crudOperation: 'export_manual',
      changes: { exportTypes, totalRecords, fileName: zipFileName, dateRange },
      ipAddress: req.ip
    }).catch(() => {});
    ...
    res.json({
      message: 'Marketing export successful',
      fileName: zipFileName,
      totalRecords,
      downloadUrl: `data:application/zip;base64,${base64Data}`,
      exportId: exportRecord._id
    ...
    console.error('Error marketing export:', error);
