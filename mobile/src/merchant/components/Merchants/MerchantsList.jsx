...
export default function MerchantsList({ merchantData }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.contactPerson || formData.contactPerson.trim().length === 0) {
      setAlert({ type: 'error', message: 'Contact person is required' });
      return;
    }
    ...
    try {
      ...
      if (editingMerchant) {
        ...
        const response = await fetch(`${API_URL}/api/merchant/merchants/${editingMerchant._id}`, {
          method: 'PUT',
          headers: {
            'Authorization': `Bearer ${token}`,
  ...
  const handleToggleActive = async (merchant) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/merchant/merchants/${merchant._id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleDelete = async (merchantId) => {
    if (!window.confirm(`Permanently delete this merchant? This cannot be undone.`)) {
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/merchant/merchants/${merchantId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
          ...
          >
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ display: 'grid', gap: '16px' }}>
                ...
                <FormField
                  label={editingMerchant ? '6-Digit PIN (leave blank to keep current)' : '6-Digit PIN'}
                  type="password"
              ...
              <div style={{
                ...
                {editingMerchant && (
                  <button
                    type="button"
                    onClick={() => handleDelete(editingMerchant._id)}
                    style={{
                      padding: '12px 24px',
