// src/components/layouts/AccountingHeader.jsx
// Accounting-specific header matching Motorpool design patterns

import React, { useState, useRef, useEffect } from 'react';
import { useTheme } from '../../context/ThemeContext';
import ThemeToggle from '../common/ThemeToggle';

export default function AccountingHeader({ adminData, onLogout, onOpenProfile }) {
  ...
  const getFullName = () => {
    if (!adminData) return 'Accounting Admin';
    return `${adminData.firstName || ''} ${adminData.lastName || ''}`.trim() || 'Accounting Admin';
  };
          ...
          {showDropdown && (
            <div style={{
              ...
              <div style={{ padding: '12px 16px', borderBottom: `1px solid ${theme.border.primary}`, marginBottom: '8px' }}>
                ...
                <div style={{ fontSize: '12px', color: theme.text.secondary }}>{adminData?.email || 'accounting@nu.edu.ph'}</div>
                <div style={{ fontSize: '10px', color: theme.accent.primary, textTransform: 'uppercase', fontWeight: 600, marginTop: '6px', letterSpacing: '0.5px' }}>Accounting Admin</div>
              </div>
