// src/components/layouts/MarketingHeader.jsx
// Marketing-specific header matching the other admin dashboards.
...
export default function MarketingHeader({ adminData, onLogout, onOpenProfile }) {
  ...
  const getFullName = () => {
    if (!adminData) return 'Marketing Admin';
    return `${adminData.firstName || ''} ${adminData.lastName || ''}`.trim() || 'Marketing Admin';
  };
  ...
  return (
    <header className="px-4 md:px-10" style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
        <div style={{
          width: '45px', height: '45px', background: theme.accent.primary, borderRadius: '12px',
        ...
        <div>
          ...
          <p className="hidden sm:block" style={{ fontSize: '12px', color: theme.accent.primary, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '1px', margin: '2px 0 0 0' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
        <div style={{
          display: 'flex', alignItems: 'center', gap: '8px', background: 'rgba(34, 197, 94, 0.2)',
          padding: '6px 14px', borderRadius: '20px', border: '1px solid rgba(34, 197, 94, 0.3)'
        }}>
        ...
        <div style={{ position: 'relative' }} ref={dropdownRef}>
          <button
            ...
            style={{
              ...
              border: `2px solid ${theme.accent.primary}`, borderRadius: '25px',
              padding: '4px 12px 4px 4px', cursor: 'pointer', transition: 'all 0.2s ease'
            }}
            onMouseEnter={(e) => { e.currentTarget.style.background = isDarkMode ? 'rgba(255, 212, 28, 0.2)' : 'rgba(59, 130, 246, 0.2)'; e.currentTarget.style.transform = 'scale(1.02)'; }}
            onMouseLeave={(e) => { e.currentTarget.style.background = isDarkMode ? 'rgba(255, 212, 28, 0.1)' : 'rgba(59, 130, 246, 0.1)'; e.currentTarget.style.transform = 'scale(1)'; }}
          >
            <div style={{
              width: '32px', height: '32px', background: theme.accent.primary, borderRadius: '50%',
              display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: '14px',
              color: isDarkMode ? '#181D40' : '#FFFFFF', flexShrink: 0
            }}>
              {getInitials()}
            </div>
          ...
          {showDropdown && (
            <div style={{
              ...
              borderRadius: '12px', padding: '8px 0', minWidth: '220px',
              ...
              <div style={{ padding: '12px 16px', borderBottom: `1px solid ${theme.border.primary}`, marginBottom: '8px' }}>
                ...
                <div style={{ fontSize: '12px', color: theme.text.secondary }}>{adminData?.email || 'marketing@nu.edu.ph'}</div>
                <div style={{ fontSize: '10px', color: theme.accent.primary, textTransform: 'uppercase', fontWeight: 600, marginTop: '6px', letterSpacing: '0.5px' }}>Marketing Admin</div>
              </div>
