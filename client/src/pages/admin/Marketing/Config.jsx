// src/pages/admin/Marketing/ConfigPage.jsx
// Marketing Configurations - matches Motorpool design with combined export archives
...
export default function ConfigPage() {
  ...
  const loadExportArchive = async () => {
    try {
      // Load both scheduled and manual exports into one archive - filtered by marketing admin role
      const [scheduledData, historyData] = await Promise.all([
        api.get('/admin/configurations/scheduled-exports?adminRole=marketing').catch(() => []),
        api.get('/admin/configurations/export-history?adminRole=marketing').catch(() => [])
      ]);
  ...
  const handleSaveAutoExport = async () => {
    setLoading(true);
    try {
      // Must include adminRole so the schedule is saved under 'marketing'
      // (without it, the server defaults to 'global' and the cron never runs it).
  ...
  const handleManualExport = async () => {
    ...
    try {
      ...
      const result = await api.post('/admin/configurations/manual-export-marketing', {
      ...
      try {
        ...
        await logDataExport({
          ...
          exportType: 'marketing_manual_export',
          ...
          fileName: result.fileName || `marketing_export_${new Date().toISOString().split('T')[0]}.zip`
        });
      ...
      toast.success(`Marketing data exported successfully! (${result.totalRecords || 0} total records)`);

      if (result.downloadUrl) {
        const link = document.createElement('a');
        link.href = result.downloadUrl;
        link.download = result.fileName || `marketing_export_${new Date().toISOString().split('T')[0]}.zip`;
    ...
    <div className="h-full flex flex-col">
      ...
      <div style={{ marginBottom: '24px', borderBottom: `2px solid ${isDarkMode ? 'rgba(255,212,28,0.2)' : 'rgba(59,130,246,0.2)'}`, paddingBottom: '16px' }}>
        <h2 style={{ color: theme.accent.primary }} className="text-2xl font-bold m-0 mb-2 flex items-center gap-[10px]">
          <Settings className="w-5 h-5" /> Marketing Settings
        </h2>
        <p style={{ color: theme.text.secondary }} className="text-[13px] m-0">
          Configure automated marketing data exports and system settings
        </p>
      </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
            <div>
              <h3 style={{ color: theme.accent.primary, fontSize: '20px', fontWeight: 700, margin: '0 0 8px 0', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <BarChart3 className="w-5 h-5" /> Automated Marketing Export
              </h3>
              <p style={{ color: theme.text.secondary, fontSize: '14px', margin: 0, lineHeight: '1.5' }}>
                System automatically exports marketing data as ZIP files for backup and compliance.
          ...
          <div style={{
            ...
            <h4 style={{ color: theme.text.primary, fontSize: '16px', fontWeight: 700, marginBottom: '8px' }}>
              Export Marketing Data Now
            </h4>
            <p style={{ color: theme.text.secondary, fontSize: '13px', marginBottom: '16px' }}>
              Download a complete ZIP file with all marketing data: transactions, cash-ins, balances, and logs.
            </p>
            <button
            ...
            >
              {loading ? '⏳ Exporting...' : '📥 Download Marketing Data (ZIP)'}
            </button>
          </div>
        </div>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '6px' }}>
                        ...
                        <span style={{ color: theme.accent.primary, fontWeight: 700, fontSize: '15px' }}>
                          {exp.exportType || 'Marketing Export'}
                        </span>
                        <span style={{
                          padding: '4px 10px',
                          borderRadius: '6px',
      ...
      {showConfigModal && (
        <ScheduleExportModal
          ...
          exportTypes={MARKETING_EXPORT_TYPES}
          loading={loading}
