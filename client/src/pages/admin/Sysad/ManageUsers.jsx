...
export default function ManageUsers() {
  ...
  // Status feedback goes through toasts (same as the rest of the admin UI)
  const showNotification = (type, title, message) => {
    const fn = toast[type === 'warning' ? 'warn' : type] || toast.info;
    fn(message ? `${title} — ${message}` : title);
  };
  ...
  const handleResetPin = (user) => {
    ...
    showConfirm(
      ...
      async () => {
        setConfirmModal({ ...confirmModal, isOpen: false });
        try {
          const res = await api.post(`/admin/sysad/users/${user._id}/reset-pin`);
          if (res.emailSent) {
            toast.success(`Temporary PIN sent to ${user.email}`);
          } else {
            // Email failed — show the PIN so the admin can hand it over in person
            toast.warn(res.message || 'Email failed — check the server logs for the PIN', { autoClose: false });
          }
          fetchUsers(true);
        } catch (error) {
  ...
  return (
    <div className="min-h-[calc(100vh-220px)] flex flex-col">
      ...
      {/* Add User Modal (shared with Dashboard; it toasts its own success incl. PIN email status) */}
      {showAddModal && (
        <AddUserModal
          theme={theme}
